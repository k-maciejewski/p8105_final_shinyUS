...
```{r convert lat&long to state name}
...
us_sentiments = us_tweets %>%
  ...
  gather(key = sentiment, value = score, anger:positive) %>%
...
```{r, map overall sentiment sums per state, eval = F}
...
plot_geo(us_sentiments) %>%
    add_trace(
      z = ~positive, text = ~hover, locations = ~abr,
      color = ~positive, colors = 'Reds') %>%
  layout(title = 'Positive Sentiment Sums across the U.S.'
         geo = g)

plot_geo(us_sentiments, locationmode = 'USA-states') %>%
    add_trace(
      z = ~negative, text = ~hover, locations = ~abr,
      color = ~negative, colors = 'Purples') %>%
  layout(title = 'Negative Sentiment Totals across the U.S.',
         geo = g)
```
...
us_tweets_long %>%
  filter(country == "US")%>% filter(hour == input$hour_choice) %>%  filter(sentiment == input$sentiment_choice)  %>% mutate(text_label = str_c("sentiment: ", sentiment, '\nlocation: ', place_as_appears_on_bio)) %>% 
  plot_ly(x = ~longitude, y = ~latitude, type = "scatter", mode = "markers",
          alpha = 0.5, 
          color = ~sentiment, text = ~text_label)
})

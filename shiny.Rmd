...
radioButtons("state_sent", label= h3("Select Positive or Negative Sentiment Totals"), choices = state_pos_neg, selected = NULL,
...
plot_geo(us_sentiments) %>%
  filter(neg_pos == input$state_sent) %>%
    add_trace(
      z = ~neg_pos, text = ~hover, locations = ~abr,
